...
extension ViewController {
    ...
    func placeDiskAfterAnimation(_ disk: Disk, atX x: Int, y: Int, completion: ((Bool) -> Void)? = nil) throws {
...
extension ViewController {
    ...
    func playTurnOfComputer() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
            ...
            try! self.placeDiskAfterAnimation(turn, atX: x, y: y) { [weak self] _ in
                self?.nextTurn()
            }
        }
...
extension ViewController: BoardViewDelegate {
    ...
    func boardView(_ boardView: BoardView, didSelectCellAtX x: Int, y: Int) {
        ...
        try? placeDiskAfterAnimation(turn, atX: x, y: y) { [weak self] _ in
            self?.nextTurn()
        }
    }
}
